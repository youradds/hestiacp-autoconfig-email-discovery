...
if ($0 ne "/installer/add-autodiscover-all.cgi") {
...
sub do_setup {
	...
	if ($dns_type eq "linode") {
		...
		`$HESTIA_PATH/v-add-dns-record $username $domain autodiscover CNAME $domain`;
		`$HESTIA_PATH/v-add-dns-record $username $domain autoconfig CNAME $domain`;
	}
